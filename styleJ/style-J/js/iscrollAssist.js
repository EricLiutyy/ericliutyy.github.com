;(function($,window){
	...
	var exportsMethods = {
		...
		newVerScrollForPull : function(wrapper,pulldownAction,pullupAction,opts,pullText){
			...
			var pulldownRefresh   = pullText && pullText['pulldownRefresh'] ? pullText['pulldownRefresh'] : '正在为您重新加载...',
				pullupLoadingMore = pullText && pullText['pullupLoadingMore'] ? pullText['pullupLoadingMore'] : '加载更多...',
				releaseToRefresh  = pullText && pullText['releaseToRefresh'] ? pullText['releaseToRefresh'] : '正在为您重新加载...',
				releaseToLoading  = pullText && pullText['releaseToLoading'] ? pullText['releaseToLoading'] : '正在为您加载...',
				loading 		  = pullText && pullText['loading'] ? pullText['loading'] : '加载中...',
				nomore            = pullText && pullText['nomore'] ? pullText['nomore'] : '没有更多信息了.';
			...
			scrollObj.on('refresh',function(){
				...
				if ($pulldown.length>0 && $pulldown.hasClass('loading')) {
					$pulldown.removeClass('flip').removeClass('loading').remove('nomore');
					$pulldown.find('.pulldown-label').html(pulldownRefresh);
				} else if ($pullup.length>0){
					$pullup.find('.pullup-icon').show();
					if($pullup.hasClass('loading')){
						$pullup.find('.pullup-icon').show();
						$pullup.removeClass('flip').removeClass('loading').remove('nomore');
						$pullup.find('.pullup-label').html(pullupLoadingMore);
					}
				}
				
				//this.y = pulldownOffset;
			});
			...
			scrollObj.on('scrollMove',function(){
				...
				if ($pulldown.length>0 && this.y > 5 && !$pulldown.hasClass('flip')) {
					$pulldown.removeClass('flip').removeClass('loading').addClass('flip');
					$pulldown.find('.pulldown-label').html(releaseToRefresh);
					this.minScrollY = 0;
				} else if ($pulldown.length>0 && this.y < 5 && $pulldown.hasClass('flip')) {
					$pulldown.removeClass('flip').removeClass('loading');
					$pulldown.find('.pulldown-label').html(pulldownRefresh);
					this.minScrollY = -pulldownOffset;
				//this.y < this.minScrollY代表是上拉,以防下拉的时候未拉到尽头时进入上拉的逻辑中
				} else if ($pullup.length>0 && this.y < this.minScrollY && this.y < (this.maxScrollY - 5) && !$pullup.hasClass('flip') && !$pullup.hasClass('nomore')) {
					$pullup.removeClass('flip').removeClass('loading').addClass('flip');
					$pullup.find('.pullup-label').html(releaseToLoading);
					this.maxScrollY = this.maxScrollY;
				} else if ($pullup.length>0 && (this.y > (this.maxScrollY + 5)) && $pullup.hasClass('flip') && !$pullup.hasClass('nomore')) {
			...
			scrollObj.on('scrollEnd',function(){
				...
				if ($pulldown.length>0 && $pulldown.hasClass('flip')) {
					$pulldown.removeClass('flip').removeClass('loading').addClass('loading');
					...
					$pullup.removeClass('flip').removeClass('loading').addClass('loading');
